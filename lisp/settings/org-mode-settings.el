...
(defun tex-add-youtube-url (url)
  ...
  (let* ((id (or (yt-extract-video-id url)
         ...
         (filename (format "~/Downloads/%s.jpeg" id))
         (title (shell-command-to-string (format "webpage-title.sh '%s'" url))))
    (message "downloading youtube thumbnail from url: %s" image-url)
    (url-copy-file image-url filename 0)
    (insert (format "\\href{%s}" url))
    (newline-and-indent)
    (insert (format "{" url))
    (tex-include-graphics filename)
    (newline-and-indent)
    (insert title)
    (insert "}")))
